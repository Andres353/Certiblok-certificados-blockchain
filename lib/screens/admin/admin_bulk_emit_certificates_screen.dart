// lib/screens/admin/admin_bulk_emit_certificates_screen.dart
// Pantalla para emitir múltiples certificados a la vez (Administrador)
// Permite seleccionar cualquier carrera de la institución
...
class AdminBulkEmitCertificatesScreen extends StatefulWidget {
  ...
  @override
  _AdminBulkEmitCertificatesScreenState createState() => _AdminBulkEmitCertificatesScreenState();
}

class _AdminBulkEmitCertificatesScreenState extends State<AdminBulkEmitCertificatesScreen> {
  bool _isLoading = true;
  bool _isEmitting = false;
  bool _isLoadingStudents = false;
  List<Map<String, dynamic>> _students = [];
  List<Map<String, dynamic>> _careers = [];
  Set<String> _selectedCareerIds = <String>{}; // Cambiar a Set para múltiples carreras
  ...
  Future<void> _loadStudents() async {
    if (_selectedCareerIds.isEmpty) {
      setState(() {
        ...
        _studentCustomCertificates.clear();
      });
      return;
    }
    ...
    try {
      ...
      // Cargar estudiantes de todas las carreras seleccionadas
      List<Map<String, dynamic>> allStudents = [];
      
      for (String careerId in _selectedCareerIds) {
        ...
        final careerName = _careers.firstWhere((c) => c['id'] == careerId)['name'];
      ...
      setState(() {
        ...
        // Mantener solo los estudiantes seleccionados que aún existen
        _selectedStudentIds = _selectedStudentIds
            .where((id) => _students.any((s) => s['id'] == id))
            .toList();
      ...
      for (String studentId in _selectedStudentIds) {
        ...
        try {
          ...
          final certificateData = {
            ...
            'issuedByName': userContext?.userName ?? 'Administrador',
            'issuedByRole': userContext?.userRole ?? 'admin_institution',
          ...
          try {
            final notificationResult = await CertificateNotificationService.notifyCertificateIssued(
              ...
              certificateType: _selectedCertificateType,
              institutionName: userContext?.currentInstitution?.name ?? 'Tu Institución',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Emisión Masiva de Certificados - Administrador'),
        backgroundColor: Color(0xff6C4DDC),
        foregroundColor: Colors.white,
        actions: [
  ...
  Widget _buildCareerSelector() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            'Seleccionar Carreras (${_selectedCareerIds.length}/${_careers.length})',
            style: TextStyle(
              fontSize: 16,
                ...
                itemBuilder: (context, index) {
                  ...
                  return Container(
                    ...
                    child: CheckboxListTile(
                      ...
                      onChanged: (value) {
                        setState(() {
                          if (value == true) {
                            ...
                            // Remover estudiantes de esta carrera de la selección
                            _selectedStudentIds.removeWhere((studentId) {
                              final student = _students.firstWhere(
                                (s) => s['id'] == studentId,
                                orElse: () => {},
                              );
                              return student['programId'] == careerId;
                            });
                            // Remover PDFs de estudiantes de esta carrera
                            _studentCustomCertificates.removeWhere((key, value) {
                              final student = _students.firstWhere(
                                (s) => s['id'] == key,
                                orElse: () => {},
                              );
                      ...
                      subtitle: Text(
                        career['facultyName'] as String? ?? 'Sin facultad',
                        style: TextStyle(
                          fontSize: 12,
  ...
  Widget _buildStudentsList() {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text(
              'Selecciona una o más carreras para ver los estudiantes',
    ...
    // Filtrar estudiantes basándose en la búsqueda
    final searchQuery = _searchController.text.toLowerCase().trim();
    final filteredStudents = searchQuery.isEmpty
        ? _students
        : _students.where((student) {
    ...
    return Column(
      children: [
        // Header con controles y buscador
        Container(
          padding: EdgeInsets.all(16),
          decoration: BoxDecoration(
            color: Colors.white,
          ...
          child: Column(
            children: [
              ...
              if (searchQuery.isNotEmpty) ...[
                SizedBox(height: 8),
                Text(
                  'Mostrando ${filteredStudents.length} de ${_students.length} estudiantes',
                  style: TextStyle(
                    fontSize: 12,
                    color: Colors.grey[600],
